  ...
  return {
    ...
    relationships: [
      ...
      },
      {
        ...
        target: 'urn:mif:topic:artifact-authoring:subagents',
      },
    ],
    extensions: {
      artifactAuthoring: {
...
test('Task #90: the worked example draft\'s actual provenance record is accepted', () => {
  ...
  const { parentSkillOrCommand, dependsOnToolSchemas } = frontmatter.extensions.artifactAuthoring;
...
test('Task #90: dropping the drafted parentSkillOrCommand is actually caught (not vacuously true)', () => {
  const frontmatter = draftWorkedExampleFrontmatter();
  const { dependsOnToolSchemas } = frontmatter.extensions.artifactAuthoring;
  assert.throws(
