...
// and computing agreement between a judge's verdicts and the golden set's
// recorded labels. This module has no way to know whether those labels are
// independently human-labeled or self-labeled by the same session that also
// produced the judge verdicts (this repo's first calibration run is the
// latter — see skills/grade-artifact/SKILL.md's "Known limitation") — that
// provenance distinction lives outside this module's scope, not enforced
// here.
...
import { ARTIFACT_TYPES } from './xdg-store.mjs';
...
function assertValidGoldenSet(data, sourcePath) {
  ...
  if (!data.criteria || typeof data.criteria !== 'string') {
    throw new Error(`${sourcePath}: missing or invalid "criteria" (required — grade-artifact reads it)`);
  }
  ...
  for (const entry of data.entries) {
    if (typeof entry !== 'object' || entry === null || Array.isArray(entry)) {
      throw new Error(`${sourcePath}: every entry must be an object, got ${JSON.stringify(entry)}`);
    }
...
export function loadGoldenSet(artifactType, { goldenSetsDir = DEFAULT_GOLDEN_SETS_DIR } = {}) {
  // artifactType is checked against the fixed known set BEFORE it's
  // interpolated into a path — otherwise a value like "../../etc/passwd"
  // (minus the .json it'd need) could read a file outside goldenSetsDir.
...
/**
 * Compare a judge's verdicts against a golden set's recorded labels (see
 * this module's header comment for the human-vs-self-labeled caveat).
...
export function computeAgreement(goldenSet, judgeVerdicts) {
  ...
  for (const entry of goldenSet.entries) {
    ...
    if (verdict === entry.label) {
      ...
      mismatches.push({ id: entry.id, goldenLabel: entry.label, judgeVerdict: verdict });
    }
  }
